...
This game has three modes: singleplayer, multiplayer, and multiplayer in Dark mode. In singleplayer mode, the player plays against an AI player (implemented using Minimax with alpha-beta pruning); in multiplayer, two players can connect to one server using socket and play against each other. After selecting the mode, the user will be able to rearrange their layout. When layout is finished, the game starts. Capture the flag to win!
